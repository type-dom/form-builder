...
export class ControlTitleProperty extends PropertyInput {
  ...
  constructor(public parent: ControlProperty | FieldProperty) {
    ...
    //   this.subject = FormDesigner.controlSubject;
    // } else {
    //   this.subject = FormDesigner.fieldSubject;
    // }
  }
  ...
  initEvents() {
    ...
    if (this.parent instanceof ControlProperty) {
      this.events.push(
        this.formBuilder.controlSubject
          .pipe(distinctUntilChanged())
          .subscribe((control) => this.listenControl(control))
      );
    } else {
      this.events.push(
        this.formBuilder.fieldSubject
          .pipe(distinctUntilChanged())
  ...
  controlPropertyReset(value?: string): void {
    if (value !== undefined) {
      this.formBuilder.selectedControl?.resetLabelText(this.content.dom.value);
      return;
    }
  }
  ...
  fieldPropertyReset(value?: string): void {
    ...
    if (value !== undefined) {
      ...
      const tableHeader =
        this.formBuilder.selectedTableDataCell.parent.parent.config
          ?.tableHeader;
      const index = this.formBuilder.selectedTableDataCell.index;
      if (tableHeader && index !== undefined) {
        tableHeader[index].label = value;
        // 修改表头标签
        (table.childNodes[0] as TableHead).childNodes[
          index
        ].childNodes[0].setText(value);
