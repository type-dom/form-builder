...
import { Span, TextNode, TypeTable, toJSON } from '@type-dom/framework';
import { FormBuilder } from '../../../../../form-builder';
import { TypeControl } from '../../../../../core/control/type-control.abstract';
import {
  ITableConfig,
  ITableField,
} from '../../../../../core/control/complex/table/table.interface';
...
export class Table extends TypeTable implements ITable {
  ...
  setTable(config: ITableConfig): void {
    ...
    config.tableData
      .map((item) => {
        // 根据表头过滤字段 todo 这样如果表头设置和行数据不对应，行数据会丢失。
      ...
      .forEach(
        (tr: Record<string, string | number | boolean>, index: number) => {
          if (index === 0) {
  ...
  createInstance(tableLiteral: ITable): void {
    ...
    this.formBuilder.mode.tableCreateInstance(this, tableLiteral);
  }
