...
import { TypeDiv, toJSON } from '@type-dom/framework';
...
import { fieldConfig } from '../control/type-control.const';
...
export class WebDocument extends TypeDiv implements IWebDocument {
  ...
  get formIds(): string | undefined {
    return this.attrObj['form-ids']
      ? (this.attrObj['form-ids'] as string)
      : undefined;
  }
  ...
  createInstance(docLiteral: IWebDocument): void {
    if (docLiteral.propObj) {
      this.resetPropObj(docLiteral.propObj);
    }
    // 处理就版中的SelectGroup对象。
  ...
  transformOldJson(json: any): boolean {
    ...
    if (json.childNodes[0].className !== 'WebDocumentTabs') {
      ...
      this.clearChildren();
      contentsJson.childNodes = json.childNodes.filter(
        (child: any) => child.className === 'WebPage'
      ) as IWebPage[];
