...
export class SelectControl extends BasicControl implements ISelectControl {
  ...
  constructor(parent: WebPage | TableDataCell) {
    ...
    const optionsConfig = deepClone(
      Object.assign(defaultOptionConfig, { name: '下拉选择' + Math.random() })
    );
