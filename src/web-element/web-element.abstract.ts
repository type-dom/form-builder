...
export abstract class WebElement implements IWebElement {
  ...
  childNodes: Array<WebElement | WebTextNode>;
  ...
  get firstChild(): WebElement | WebTextNode {
    return this.childNodes[0];
  }

  get lastChild(): WebElement | WebTextNode {
  ...
  // 设置属性
  setPropObj(propObj: IWebProperty): void {
  ...
  // 设置属性 dom 属性同步变化
  setAttribute(key: string, value: string| number | boolean): WebElement {
    this.addAttribute(key, value);
    this.renderAttribute(key, value);
    return this;
  }
  ...
  setAttrName(value: string): WebElement {
  ...
  removeAttribute(key: string): WebElement {
  ...
  addClassName(className: string): WebElement {
    // 要先判断className是否已经存在
  ...
  removeClassName(className: string): WebElement {
    String(this.propObj.attrObj.class).replace(className, '');
    this.dom.classList.remove(className);
    return this;
  }
  // todo 真实的dom操作
  // abstract setParent(parent: WebElement): void;
   ...
   * @param newChild
   */
  // abstract appendChild(newChild: WebElement | WebTextNode): WebElement | WebTextNode;
  appendChild(newChild: WebElement| WebTextNode): void {
  ...
  addChild(newChild: WebElement | WebTextNode): void {
    this.childNodes.push(newChild);
  }
  /**
   ...
   * @param newChild
   */
  renderChild(newChild: WebElement | WebTextNode): void {
    newChild.render();
    this.dom.appendChild(newChild.dom);
  }

  // appendChildren(newNodes: Array<WebElement | WebText>) {
   ...
   */
  // insertChild(child: WebElement | WebTextNode, index: number) {
   ...
   * @param index 数组下标
   */
  insertChild(newChild: WebElement | WebTextNode, index: number): HTMLElement | SVGElement | Text {
  ...
  // insertChildren(children: Array<WebElement | WebText>, index: number) {
  ...
  // 替换
  // replaceChild(newNode: WebElement, oldNode: WebElement): void {
  ...
  findChildAtIndex(index: number): WebElement | WebTextNode | null {
    return this.childNodes[index] || null;
  }

  findChildIndex(child: WebElement | WebTextNode): number {
   ...
   * @param literal
   */
  createInstance(literal: IWebElement): void {
