import { IComponent, INodeAttr, IWebNode } from './web-node.interface';
export abstract class WebNode implements IWebNode {
  nodeName: string;
  nodeValue: string;
  parentNode: WebNode | null;
  childNodes: WebNode[];
  ...
  get firstChild(): WebNode {
    return this.childNodes && this.childNodes[0];
  }
  get nextSibling(): WebNode | undefined {
   ...
   */
  searchNode(paths: IComponent[], pos: number): WebNode | null {
    if (pos >= paths.length) {
      return this;
    }
    const component = paths[pos];
    const stack: [WebNode, number][] = [];
    // eslint-disable-next-line @typescript-eslint/no-this-alias
    let node: WebNode = this;
    while (true) {
