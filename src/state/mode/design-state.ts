    ...
    if (control.parent instanceof WebPage) {
      if (control.parent.dragDropIndex !== undefined) {
        ...
        if (control.parent.dragStartIndex > control.parent.dragDropIndex) {
          ...
          control.parent.childNodes.splice(control.parent.dragDropIndex, 0, control);
          control.parent.childNodes.splice(control.parent.dragStartIndex + 1, 1);
        } else {
          // 如果当前元素在拖动目标位置的上方，先将当前元素从数组拿出，数组长度-1，我们直接给数组拖动目标位置+1的地方新增一个和当前元素值一样的元素，
          // 这时，数组len不变，我们再把数组之前的那个拖动的元素删除掉，下标还是index
          control.parent.childNodes.splice(control.parent.dragDropIndex + 1, 0, control);
